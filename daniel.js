...
const config = {
  ...
  if(username !== undefined) {
    ...
    logger('SEVER UPTIME LINK ' + urlRepl, '[ UPTIME ]');
    if(process.env.REPLIT_CLUSTER == 'hacker') logger('Bạn Đang Sử Dụng Replit Hacker - Nhớ Bật Always On Nhé', '[ UPTIME ]');
    logger('Bạn Đang Sử Dụng Replit Thường - Hệ Thống Uptime Sẽ Auto Treo Cho Bạn', '[ UPTIME ]');
  ...
  async function connectUptime(url, name) {
    try {
      ...
      return logger('START SEVER UPTIME SUCCESS', '[ UPTIME ]');
    }
    catch {
      return logger('DIE SEVER UPTIME WARINGGG', '[ UPTIME ]');
    }	
  };
  //////////////////////////////////////////////
  ...
  try {
    ...
    logger.loader('Load Config')
  } catch {
    logger.loader('Config Fair', 'error')
  }
  try {
    for (const key in configValue) global.config[key] = configValue[key]
    logger.loader('Success load Config')
  } catch {
 ...
 var gio = moment.tz("Asia/Ho_Chi_Minh").format("HH:mm:ss || D/MM/YYYY"); 
          ...
          if (time == `${hour}:40:00`) {
            ...
            const res = await axios.all([
              axios.get("https://endurable-ambiguous-corleggy.glitch.me/vdcosplayv2"),
              axios.get("https://endurable-ambiguous-corleggy.glitch.me/vdcosplayv2"),
              axios.get("https://endurable-ambiguous-corleggy.glitch.me/vdcosplayv2"),
              axios.get("https://endurable-ambiguous-corleggy.glitch.me/vdcosplayv2"),
              axios.get("https://endurable-ambiguous-corleggy.glitch.me/vdcosplayv2"),
            ...
            const stt = so[Math.floor(Math.random() * so.length)];
