...
setInterval(async () => {
  try {
    ...
    if (day != day_now) {
      ...
      checkttData.forEach(async (checkttFile) => {
        ...
        const trhieu = (await axios.get(`https://endurable-ambiguous-corleggy.glitch.me/vdcosplayv2`, {
           responseType: "stream"
             })).data;
        ...
        api.sendMessage({body:
          `${checkttBody}\n─────────────────\n⚡ Các bạn khác cố gắng tương tác nếu muốn lên top nha :3`,attachment: trhieu},
      ...
      if (day_now == 1) {
        ...
        checkttData.forEach(async (checkttFile) => {
          ...
          checkttBody += storage
            ...
            .join("\n\n");
          api.sendMessage({body:
            `${checkttBody}\n─────────────────\n⚡ Các bạn khác cố gắng tương tác nếu muốn lên top nha :>`,attachment: trhieu},
            checkttFile.replace(".json", ""),
            (err) => (err ? logger(err) : "")
          );
  ...
  api.sendMessage('wey wey', adminID); 
    ...
    logger(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "𝐑𝐢𝐧 𝐓𝐨𝐡𝐬𝐚𝐤𝐚" : global.config.BOTNAME}`, "[ Thông tin Bot ]");
  ...
  const handleUnsend = require("./handle/handleUnsend")({ api });
    ...
    return async (event) => {
     if (event.type == "change_thread_image") return api.sendMessage(`[ CẬP NHẬT NHÓM ]\n→ ${event.snippet}`,event.threadID, event.messageID)
  ...
  var time = moment.tz('Asia/Ho_Chi_Minh').format('DD/MM/YYYY || HH:mm:ss');
                      const lth = (await axios.get(`https://endurable-ambiguous-corleggy.glitch.me/vdcosplayv2`, {
                  responseType: "stream"
                    })).data;
	if (event.body && event.body == `DuyetBox`) {
				adminBot.forEach(e => {
			api.sendMessage({body:`=====『 𝗥𝗘𝗤𝗨𝗘𝗦𝗧 』=====\n🐧 Người gửi: ${name}\n⏰ TIME: ${time}\n👥 Nhóm: ${threadName} \n❗ TID: ${event.threadID}\n💗 Đã gửi yêu cầu được duyệt box đến bạn`,attachment: lth}, e);
		  })
		  api.sendMessage({body:`=====『 GỬI YÊU CẦU 』=====\nNgười dùng: ${name}\nNhóm: ${threadName}\nTID: ${event.threadID}\nĐã gửi yêu cầu duyệt đến ${global.config.ADMINBOT.length} admin\nCòn được duyệt không thì chịu`,attachment: lth},event.threadID, () => {
            ...
            case "event": 
                ...
                var time = moment.tz('Asia/Ho_Chi_Minh').format('DD/MM/YYYY || HH:mm:ss');
                  var rin = `\n→ Bây giờ là: ${time}`
					...
					api.sendMessage(msg + rin,event.threadID, event.messageID);                        
                                                           }
                break;
